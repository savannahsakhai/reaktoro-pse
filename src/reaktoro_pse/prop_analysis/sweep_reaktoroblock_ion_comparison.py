...
def run_analysis(case_num=1, flowsheet=reaktoro_flowsheet_bw, water = "bw", tech = "MVC", interpolate_nan_outputs=False, output_filename=None):
    ...
    if case_num == 1:
        ...
        sweep_params["Feed Mass Frac"] = PredeterminedFixedSample(
           m.fs.feed[0].mass_frac_phase_comp["Liq", "NaCl"], [0.002, 0.035, 0.05, 0.1]
        )
        sweep_params["Mono to Di Ratio"] = PredeterminedFixedSample(
            m.fs.bw.mono_di_ratio, [0.25, 0.5, 1, 2, 4]
        )
...
if __name__ == "__main__":
    results, sweep_params, m = run_analysis(case_num=1, flowsheet=reaktoro_flowsheet_bw, tech = "MVC", output_filename="data_property_bw_reaktoro_mg.csv")
    results, sweep_params, m = run_analysis(case_num=1, flowsheet=reaktoro_flowsheet_bw, tech = "RO", output_filename="data_property_bw_reaktoro_osm_p_mg.csv")
